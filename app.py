  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", width: "100%" }}>
      ...
      <div style={{
        ...
        {/* Left column — upload + result image */}
            ...
            {preview ? (
              <>
                <img src={preview} alt="preview" style={{
                  ...
                  display: "block", borderRadius: 12, background: "#0b1018",
                }} />
                <div style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16, position: "sticky", top: 24 }}>
          ...
          {result && (
            <div className="fu" style={{
              ...
              {/* ── MEASURE ── fixed null/undefined guards */}
              {mode === "measure" && (
                <>
                  <MetRow
                    label="Bone Height"
                    val={result.height_mm != null ? `${result.height_mm} mm` : "—"}
                    dot={C.red} col={C.red}
                  />
                  <MetRow
                    label="Width @ 2mm"
                    val={result.widths_mm?.w2mm != null ? `${result.widths_mm.w2mm} mm` : "—"}
                    dot={C.accent} col={C.accent}
                  />
                  <MetRow
                    label="Width @ 6mm"
                    val={result.widths_mm?.w6mm != null ? `${result.widths_mm.w6mm} mm` : "—"}
                    dot={C.orange} col={C.orange}
                  />
                  <MetRow
                    label="Width @ 8mm"
                    val={result.widths_mm?.w8mm != null ? `${result.widths_mm.w8mm} mm` : "—"}
                    dot={C.purple} col={C.purple}
                  />
                  <MetRow
                    label="Crest → Nerve"
                    val={result.crest_to_nerve_mm != null ? `${result.crest_to_nerve_mm} mm` : "Not computed"}
                    dot={C.yellow} col={C.yellow}
                  />
                </>
              )}
              ...
              {mode === "recommend" && (
                <>
                  <MetRow
                    label="Bone Height"
                    val={result.bone_height_mm != null ? `${result.bone_height_mm} mm` : "—"}
                    dot={C.red} col={C.red}
                  />
                  <MetRow
                    label="Bone Width"
                    val={result.bone_width_mm != null ? `${result.bone_width_mm} mm` : "—"}
                    dot={C.accent} col={C.accent}
                  />
                  <MetRow
                    label="Avail. Width"
                    val={result.implant_width_mm != null ? `${result.implant_width_mm} mm` : "—"}
                    dot={C.green} col={C.green}
                  />
                  <MetRow
                    label="Avail. Height"
                    val={result.implant_height_mm != null ? `${result.implant_height_mm} mm` : "—"}
                    dot={C.green} col={C.green}
                  />
                </>
              )}
